<template>
    <div class="centro">
        <div class="heaser-serch-add">
            ...
            <button class="addStudent oppa" @click="addStudent()">
                Add student
            </button>
        </div>
        <input class="serachNameStudents oppa" placeholder="Search" type="text" name="search" v-model="search" />
        <br />
        <table class="allStudents">
            <tr v-for="item in students" :key="item._id">
                ...
                <td class="marg">
                    <a href="#" @click="deleteStudent(item._id)">
                        Видалити
                    </a>
                </td>
                <template v-if="item._id == editId">
                    <td>
                        <input class="cock" v-model="item.name" />
                    </td>
                    <td class="marg">
                        <input type="checkbox" v-model="item.isDonePr" />
                    </td>
                    <td class="marg">{{ item.group }}</td>
                    <button class="marg upData" @click="updateStudentInfo(editId)">
                        Update student
                    </button>
                </template>
                <template v-else>
                    <!-- <td><router-link v-bind:to="'/components/Student-info.vue/' + item._id">{{
                    `${item.name}`
                    }}</router-link></td> -->
                    <td>
                        <a href="#" @click.prevent="setEditItem(item._id)">
                            Update
                        </a>
                    </td>
                </template>
